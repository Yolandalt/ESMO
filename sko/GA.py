...
class GeneticAlgorithmBase(SkoBase, metaclass=ABCMeta):
    ...
    def x2y(self):   # 限制函数处理
        self.Y_raw = self.func(self.X)  # 进行函数的转换，func_transformer
...
class GA_TSP(GeneticAlgorithmBase):
    ...
    def crtbp(self):
        ...
        self.Chrom = tmp.argsort(axis=1)  # argsort()函数是将x中的元素从小到大排列，提取其对应的index(索引号)
        return self.Chrom
...
class GA_EdgeVideo(GeneticAlgorithmBase):
    ...
    def crtbp(self):
        # 是不是可以利用上次最优的卸载决策进行初始化化
        # create the population(初始化种群)
        # 种群为种群数*染色体长度，值为（0~7之间的随机数，代表卸载到不同的计算节点）
        # 根据给定的模型缓存决策，进行初始化任务卸载决策
        # num_model_in_edge = []  # 每个边缘节点上的模型数量
        # num_model_in_edge.append(3)  # 卸载到本地的概率
        # for edge in range(len(self.caching_decision)):
        #     num_model_in_edge.append(len(self.get_index1(self.caching_decision[edge])))
        # # 产生随机数的概率，模型数越多，卸载到那个节点的概率越大
        # # 卸载到云端的概率
        ...
        if len(self.last_best_offloading_decision):  # 非空，不是第一次
            self.Chrom[0] = self.last_best_offloading_decision  # 替换为上一轮最优的卸载决策
            print("非空，替换最优的决策")
        print("self.Chrom")
        print(self.Chrom)
        return self.Chrom
...
class GA_EdgeVideo_Y(GeneticAlgorithmBase):
    ...
    def crtbp(self):
        # create the population(初始化种群)
        # 种群为种群数*染色体长度，值为（0~1之间的随机数，代表不同的缓存模型）
        # 卸载的数量越多越应该缓存模型
        # num_frame_in_edges = np.zeros(self.n_edge)
        # for user in range(len(self.offloading_decision)):
        #     for frame in range(len(self.offloading_decision[user])):
        #         for edge in range(self.n_edge):
        #             if self.offloading_decision[user][frame] == edge+1:
        #                 num_frame_in_edges[edge]+=1
        # rate = num_frame_in_edges / np.sum(num_frame_in_edges)
        #
        # # np.random.choice(a=range(len(a)), size=[size_pop, len_chrom], p=rate)
        # # self.Chrom = np.random.choice(a=range(len(num_frame_in_edges)), size=[self.size_pop, self.len_chrom], p=rate)
        # # 根据概率计算每个边缘节点缓存CNN模型的数量
        #
        # List = []
        # num_CNNModel_in_edges = np.random.choice(a=range(len(num_frame_in_edges)), size=[self.size_pop, len(num_frame_in_edges)], p=rate)
        # for pop in range(self.size_pop):
        #     row = []
        #     row_edge = np.zeros(self.n_CNNmodel)
        #     for edge in range(len(num_frame_in_edges)):
        #         index_model = np.random.randint(0,int(self.n_CNNmodel/2),num_CNNModel_in_edges[pop][edge])
        #         for item in index_model:
        #             row_edge[item] = 1
        #         row = np.hstack((row,row_edge))
        #     if len(List):
